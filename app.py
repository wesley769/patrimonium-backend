...
def get_transacoes(empresa_id):
    ...
    if emp and (emp["periodo_inicio"] or emp["periodo_fim"]) and g.role != "admin":
        filters = ["empresa_id=%s"]
        params  = [empresa_id]
        if emp["periodo_inicio"]:
            filters.append("data_competencia >= %s")
            params.append(emp["periodo_inicio"])
        if emp["periodo_fim"]:
            # Pega o último dia do mês fim
            filters.append("data_competencia <= (date_trunc('month', %s::date) + interval '1 month - 1 day')::date")
            params.append(emp["periodo_fim"] + "-01")
