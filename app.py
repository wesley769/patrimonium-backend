...
def me():
    ...
    return jsonify(user=dict(**user, id=str(user["id"]), empresas=g.empresas))
...
def list_empresas():
    ...
    return jsonify(empresas=[dict(**r, id=str(r["id"])) for r in rows])
...
def create_empresa():
    ...
    db.commit()
    row = cur.fetchone()
    return jsonify(empresa=dict(**row, id=str(row["id"]))), 201
...
def get_empresa(empresa_id):
    ...
    return jsonify(empresa=dict(**row, id=str(row["id"])))
...
def get_transacoes(empresa_id):
    ...
    return jsonify(transacoes=[dict(**r, id=str(r["id"])) for r in rows])

@app.route("/empresas/<empresa_id>/transacoes", methods=["POST"])
@require_auth
@require_admin
def save_transacoes(empresa_id):
    """Recebe array de transações processadas do frontend e salva no banco."""
    data = request.get_json()
    ...
    db = get_db()
    # Limpa transações anteriores desta empresa
...
def _parse_date(v):
    ...
    if isinstance(v, str):
        try: return datetime.fromisoformat(v.replace("Z","")).date()
        except: return None
...
def list_usuarios():
    ...
    for r in rows:
        ...
        result.append(dict(**r, id=str(r["id"]), empresas=emps,
...
def create_usuario():
    ...
    for emp_id in emps:
        db.execute(
            ...
            (user_id, emp_id, d.get("role_empresa","cliente"))
        )
    db.commit()
...
def update_usuario(usuario_id):
    ...
    if d.get("empresas") is not None:
        ...
        for emp_id in d["empresas"]:
            db.execute(
                "INSERT INTO usuario_empresas (usuario_id,empresa_id,role) VALUES (%s,%s,%s)",
                (usuario_id, emp_id, d.get("role_empresa","cliente"))
            )
    if d.get("nova_senha"):
