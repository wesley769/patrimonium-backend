...
# ─── CONNECTION POOL ─────────────────────────────────────
# Troca porta 5432 → 6543 (Supavisor Transaction Mode) automaticamente
_db_url = DATABASE_URL.replace(":5432/", ":6543/") if ":5432/" in DATABASE_URL else DATABASE_URL

pool = ConnectionPool(
    conninfo=_db_url,
    min_size=2,
    max_size=10,
    open=False,          # não conecta na inicialização — evita loop de auth failure
    kwargs={"row_factory": dict_row},
)
pool.open(wait=False)    # abre em background sem bloquear o startup
atexit.register(pool.close)

def get_db():
    """Retorna a conexão do pool associada ao contexto da requisição."""
    if "db" not in g:
        g.db = pool.getconn()
    return g.db
...
def handle_500(e):
    ...
    origin = request.headers.get("Origin","")
    resp = jsonify(error="Erro interno no servidor. Tente novamente.")
...
def create_empresa():
    ...
    db = get_db()
    cur = db.execute(
        """INSERT INTO empresas (razao, fantasia, cnpj, segmento, regime, porte, cidade, estado, cor, status, periodo_inicio, periodo_fim)
...
def update_empresa(empresa_id):
    ...
    db.execute(
        """UPDATE empresas SET razao=%s,fantasia=%s,cnpj=%s,segmento=%s,regime=%s,
           porte=%s,cidade=%s,estado=%s,cor=%s,status=%s,periodo_inicio=%s,periodo_fim=%s WHERE id=%s""",
        (d.get("razao"), d.get("fantasia"), d.get("cnpj"), d.get("segmento"),
         d.get("regime"), d.get("porte"), d.get("cidade"), d.get("estado"),
         d.get("cor"), d.get("status"),
         d.get("periodo_inicio"), d.get("periodo_fim"), empresa_id)
    )
    db.commit()
...
def get_transacoes(empresa_id):
    ...
    # Busca período configurado da empresa
    emp = db.execute("SELECT periodo_inicio, periodo_fim FROM empresas WHERE id=%s", (empresa_id,)).fetchone()

    # Filtra por período se configurado e usuário não é admin
    # Filtra por período só se configurado E usuário não é admin
    periodo_inicio = emp["periodo_inicio"] if emp else None
    periodo_fim    = emp["periodo_fim"]    if emp else None

    if (periodo_inicio or periodo_fim) and g.role != "admin":
        filters = ["empresa_id=%s"]
        params  = [empresa_id]
        if periodo_inicio:
            filters.append("data_competencia >= %s")
            params.append(periodo_inicio + "-01")
        if periodo_fim:
            filters.append("data_competencia <= (date_trunc('month', %s::date) + interval '1 month - 1 day')::date")
            params.append(periodo_fim + "-01")
        query = f"SELECT * FROM transacoes WHERE {' AND '.join(filters)} ORDER BY data_competencia"
    ...
    def _fix(r): d=dict(r); d["id"]=str(d["id"]); return d
    return jsonify(transacoes=[_fix(r) for r in rows])
...
def save_transacoes(empresa_id):
    ...
    append = data.get("append", False)  # True = não apagar, só acumular
    if not txs:
...
def get_mapeamentos(empresa_id):
    ...
    return jsonify(mapeamentos=dict(
        ...
        conta_tipo_map = row["conta_tipo_map_json"],
...
def save_mapeamentos(empresa_id):
    ...
    db.execute(
        """INSERT INTO mapeamentos
           (empresa_id, column_map_json, category_map_json, conta_tipo_map_json,
            plano_contas_json, balanco_manual_json, balanco_corte_json, atualizado_em)
           VALUES (%s,%s,%s,%s,%s,%s,%s,NOW())
           ON CONFLICT (empresa_id) DO UPDATE SET
           column_map_json=%s, category_map_json=%s, conta_tipo_map_json=%s,
           plano_contas_json=%s, balanco_manual_json=%s, balanco_corte_json=%s,
           atualizado_em=NOW()""",
        (
            empresa_id,
            json.dumps(d.get("column_map",      {})),
            json.dumps(d.get("category_map",    {})),
            json.dumps(d.get("conta_tipo_map",  {})),
